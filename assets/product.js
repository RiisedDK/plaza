if (!customElements.get('variant-selects')) {
  ...
  class VariantSelects extends HTMLElement {
    ...
    getSectionsToRender() {
      return [`price-${this.dataset.section}`, `price-${this.dataset.section}--sticky`, `product-image-${this.dataset.section}--sticky`, `inventory-${this.dataset.section}`, `sku-${this.dataset.section}`, `quantity-${this.dataset.section}`, `product-label-${this.dataset.section}`];
    }
      ...
      fetch(`${this.dataset.url}?variant=${this.currentVariant.id}&section_id=${this.dataset.section}`)
        ...
        .then((responseText) => {
          ...
          sections.forEach((id) => {
            ...
            if (destination && destination.querySelector('.store-availability')) return;
    ...
    toggleAddButton(disable = false, text = false, modifyClass = true) {
      const productForm = document.getElementById(`product-form-${this.dataset.section}`);
      if (!productForm) return;
      const submitButtons = document.querySelectorAll('.single-add-to-cart-button.buy-button');
      ...
      if (disable) {
        ...
        notifyMeButtons.forEach((notifyButton) => {
...
if (!customElements.get('product-form')) {
  customElements.define('product-form', class ProductForm extends HTMLElement {
    ...
    onSubmitHandler(evt) {
      evt.preventDefault();
      if (evt.submitter.classList.contains('notify-me-btn')) return;
      ...
      fetch(`${theme.routes.cart_add_url}`, config)
        ...
        .finally(() => {
          ...
          // Remove any existing complementary items, so it does not add the existing list again
          const complementaryItems = this.form.querySelectorAll('input[data-variant-id]')
          complementaryItems.length && complementaryItems.forEach((item) => {
            item.remove()
          })

          const variantDropdowns = document.querySelectorAll('.product-variants-dropdown #variants')
          variantDropdowns.length && variantDropdowns.forEach((options) => {
            options.querySelectorAll('option').forEach((option) => {
              option.removeAttribute('selected')
            })
          })
          const variantSelectedSubmit = document.querySelectorAll('.product-variants-dropdown button')
          variantSelectedSubmit.length && variantSelectedSubmit.forEach((button) => {
